...
arduino_temp::Program::Program(Config& config) {
  config_ = &config;
  ntp_ = new NTP(config_->NTP_CONFIG);
  logger_ = new Logger(*ntp_, config_->logEnabled_);
  fileHandler_ = new FileHandler(*logger_);
  timing_ = new Timing(*logger_);
  mqtt_ = new MQTT(config_->MQTT_CONFIG, *logger_);
  wifi_ = new Wifi(config_->WIFI_CONFIG, *logger_);
  coreFunctions_ = new CoreFunctions(config_->COREFUNCTIONS_CONFIG, *logger_);
  measure_ = new Measure(config_->MEASURE_CONFIG, *logger_);
  ftp_ = new FTP(config_->FTP_CONFIG);
}
